...
// loadThemeFromFile loads a custom theme from colors.json
func loadThemeFromFile(path string) (Theme, bool) {
...
var (
	...
	colInputBg   lipgloss.Color
	colInputText lipgloss.Color
	colInputDim  lipgloss.Color
...
func initTheme() {
	// First, try to load custom colors.json
	configDir, _ := os.UserConfigDir()
	colorsPath := filepath.Join(configDir, "codebase", "colors.json")
	if t, ok := loadThemeFromFile(colorsPath); ok {
		ActiveTheme = t
		initStyles()
		return
	}

	// Fall back to built-in themes or CODEBASE_THEME env var
...
func initStyles() {
	...
	colInputBg = lipgloss.Color(t.InputBg)
	colInputText = lipgloss.Color(t.InputText)
	colInputDim = lipgloss.Color(t.InputDim)
