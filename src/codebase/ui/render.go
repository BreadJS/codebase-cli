...
func renderToolBlock(toolName string, args map[string]any, output string, state string, width int) string {
	...
	case "success":
		...
		style = theme.StyleToolSuccess
	case "error":
		statusIcon = styleErr.Render("✗")
		style = theme.StyleToolError
	default:
		statusIcon = styleWarn.Render("⣾")
		style = theme.StyleToolPending
	}

	// Build the label
	label := theme.StyleToolName.Render(toolName)
	pathStr := ""
	if args != nil {
		if p, ok := args["path"]; ok {
			if s, ok := p.(string); ok {
				pathStr = " " + theme.StyleFilePath.Render(s)
			}
		}
		...
		if toolName == "git_commit" {
			if m, ok := args["message"]; ok {
				if s, ok := m.(string); ok {
					...
					pathStr = " " + theme.StyleFilePath.Render(msg)
				}
			}
		}
...
func renderFilePreview(args map[string]any, output string, state string, width int) string {
	...
	for i, line := range lines {
		...
		lineNo := theme.StyleLineNo.Render(fmt.Sprintf(" %3d", i+1))
		sep := styleDim.Render(" │ ")
