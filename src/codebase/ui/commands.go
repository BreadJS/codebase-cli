...
func init() {
	...
	},
	{
		...
		handler: func(m *chatModel, args string) tea.Cmd {
			...
			compacted, ok := agent.CompactHistory(m.agent.Client, m.agent.History)
			if ok {
	...
	},
	{
		...
		handler: func(m *chatModel, args string) tea.Cmd {
			if args == "" {
				...
				if m.agent != nil {
					m.agent.Client = llm.NewLLMClient(m.config.APIKey, m.config.BaseURL, m.config.Model)
				}
				// Update glue clients to use new model as fallback
				if m.glue != nil {
					m.glue = llm.NewGlueClient(m.config)
				}
				m.segments = append(m.segments, segment{
					kind: "text",
	...
	},
	{
		...
		handler: func(m *chatModel, args string) tea.Cmd {
			...
			if lastText == "" {
				m.notify.Push(llm.Notification{Type: llm.NotifyWarn, Text: "Nothing to copy"})
				return nil
			}
			...
			if err := copyToClipboard(clean); err != nil {
				m.notify.Push(llm.Notification{Type: llm.NotifyWarn, Text: "No clipboard tool found (install xclip)"})
			} else {
				m.notify.Push(llm.Notification{Type: llm.NotifySuccess, Text: "Copied to clipboard"})
			}
			return nil
		},
	},
	{
		...
		handler: func(m *chatModel, args string) tea.Cmd {
			if args == "" {
				m.segments = append(m.segments, segment{
					kind: "text",
					text: styleMuted.Render("  Current theme: ") + styleAccentText.Render(theme.ActiveTheme.Name) + "\n" +
			...
			case "dark", "light":
				...
				m.segments = append(m.segments, segment{
					kind: "text",
					text: styleOK.Render("  ✓ ") + styleMuted.Render("Switched to "+theme.ActiveTheme.Name+" theme.\n"),
				})
				m.rebuildViewport()
			default:
				m.segments = append(m.segments, segment{
	...
	},
	{
		...
		handler: func(m *chatModel, args string) tea.Cmd {
			...
			default:
				...
				if m.agent.PermState.Level == types.PermTrustAll {
