...
func GenerateQuestion(glue *llm.GlueClient, prompt string, qaHistory []QAPair, questionNum int) (*PlanQuestion, bool, string) {
	...
	result, err := llm.NonStreamingChat(glue.Smart, messages)
	if err != nil {
...
func GeneratePlan(glue *llm.GlueClient, prompt string, qaHistory []QAPair) string {
	var sb strings.Builder
	sb.WriteString(fmt.Sprintf("User's request: %q\n\n", prompt))
...
func RevisePlan(glue *llm.GlueClient, currentPlan, feedback string) string {
	messages := []types.ChatMessage{
