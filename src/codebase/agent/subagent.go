...
var subagentToolDefs []types.ToolDef

func init() {
	for _, td := range tools.ToolDefs {
...
func RunSubagent(client *llm.LLMClient, workDir, task string) (string, error) {
	...
	for turn := 1; turn <= subagentMaxTurns; turn++ {
		...
		for _, tc := range toolCalls {
			...
			if tc.Function.Name == "shell" {
				...
				output, _ = tools.ExecuteTool(tc.Function.Name, tc.Function.Arguments, workDir)
			}
			history = append(history, types.ChatMessage{
				Role:       "tool",
				ToolCallID: tc.ID,
...
func executeReadOnlyShell(argsJSON string, workDir string) string {
	...
	output, _ := tools.ExecuteTool("shell", argsJSON, workDir)
	return output
}
