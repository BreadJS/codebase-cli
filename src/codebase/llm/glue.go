...
// strPtr returns a pointer to a string (convenience helper).
...
func NewGlueClient(mainCfg *types.Config) *GlueClient {
...
func (g *GlueClient) ChatReply(userMsg string, recentContext []types.ChatMessage) string {
	messages := []types.ChatMessage{
	...
	messages = append(messages, types.ChatMessage{Role: "user", Content: strPtr(userMsg)})
